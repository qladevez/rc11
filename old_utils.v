...
intros A r r' Hinc x y H. induction H.
  - apply trc_step. apply (Hinc x y). auto.
  - apply trc_ind with (z := z); auto.
Qed.
...
Lemma maybe_reflexive:
  forall A (r: Rln A) (x:A), (maybe r) x x.
Proof.
  intros. unfold maybe. right. auto.
Qed.
...
Definition imm (A: Type) (r: Rln A) : Rln A :=
  fun a => fun b =>
    r a b /\
    forall c, r c b -> (maybe r) c a /\
    forall c, r a c -> (maybe r) b c.
...
Definition res_eset (A:Type) (e: set A) (r: Rln A) :=
...
Lemma res_eset_incl : forall A (e : set A) (r : Rln A),
  rel_incl (res_eset e r) r.
Proof.
  intros A e r x y [_ [_ H]]. auto.
Qed.

(** The reflexive closure of the restriction of a relation is included in the 
reflexive closure of the relation *)

Lemma maybe_res_eset_incl: forall A (e : set A) (r : Rln A),
  rel_incl (maybe (res_eset e r)) (maybe r).
Proof.
  intros A e r x y [H | H].
  - left. apply (res_eset_incl H).
  - right. auto.
Qed.

(** The union of the restrictions of two relations to a same set is included in
the union of these sets *)

Lemma union_res_eset_incl: forall A (e : set A) (r r' : Rln A),
  rel_incl (rel_union (res_eset e r) (res_eset e r')) (rel_union r r').
Proof.
  intros A e r r' x y [H | H].
  - left. apply (res_eset_incl H).
  - right. apply (res_eset_incl H).
Qed.

(** If a relation is irreflexive, its restriction is irreflexive *)

Lemma res_eset_irr : forall A (e : set A) (r : Rln A),
  (forall x, ~ r x x) ->
  (forall x, ~ (res_eset e r) x x).
Proof.
  intros A e r H x Hnot. destruct Hnot as [_ [_ Hcont]].
  apply (H x) in Hcont. destruct Hcont.
Qed.

(** If the sequence of two relations is irreflexive, the sequence of the two
relations restricted to a same set is irreflexive *)

Lemma seq_res_eset_irr : forall A (e : set A) (r r' : Rln A),
  (forall x, ~ (rel_seq r r') x x) ->
  (forall x, ~ (rel_seq (res_eset e r) (res_eset e r')) x x).
Proof.
  intros A e r r' H x Hnot. apply (H x).
  destruct Hnot as [y [Hnot1 Hnot2]].
  exists y; split.
  - apply (res_eset_incl Hnot1).
  - apply (res_eset_incl Hnot2).
Qed.

End BasicRel.

(** A few notations for relations *)

Module RelNotations.

Notation "A ;; B" := (rel_seq A B) (at level 80, right associativity).
Notation "A ⁺" := (tc A) (at level 85).
Notation "A ?" := (maybe A) (at level 85).
Notation "A **" := (tc (maybe A)) (at level 85).
